...
for i in range(num_of_epsilon):
    ...
    for a in range(3):
        for b in range(3):
            ...
            var_epsilon[a,b] = (pi_A*(1-pi_A)/N +DY[a, b] / (L + 1 - 2 * EY[a, b]) / (L + 1 - 2 * EY[a, b]) / N) * epsilon_ab
    ...
    var_C_Theory_Analysis_L3[i] = pi_A*(1-pi_A)/N +DY[A, B] / (L + 1 - 2 * EY[A, B]) / (L + 1 - 2 * EY[A, B]) / M
...
with open('Experimental_C_L=3.txt', 'w') as file:
