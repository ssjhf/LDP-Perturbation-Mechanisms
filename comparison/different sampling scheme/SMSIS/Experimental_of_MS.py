...
N = 1000
num_of_epsilon = 50
epsilon = np.linspace(5, 10,num_of_epsilon)
...
plt.plot(epsilon, var_MS_Numerical_Simulation, color='b', marker ='o', markerfacecolor='none', linewidth=1, markersize=10, linestyle='--', label='modified Simmons(numerical simulation)')
plt.xlim(5, 10)
plt.xticks(np.linspace(5,10, num=num_of_epsilon))
...
with open('Experimental_of_MS_N=1000_pi_A=0.04.txt', 'w') as file:
    for i in range(len(epsilon)):
        line = f"{epsilon[i]}\t{var_MS_Numerical_Simulation[i]}\n"
        file.write(line)
