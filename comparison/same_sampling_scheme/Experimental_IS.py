...
var_IS_Theory_Analysis = [0] * num_of_epsilon
...
for i in range(num_of_epsilon):
    ...
    var_IS_Theory_Analysis[i] = (pi_B * (1 - p_Simmons[i]) - pi_B * (1 - p_Simmons[i]) * pi_B * (1 - p_Simmons[i])) / p_Simmons[i] / p_Simmons[i] / N
plt.plot(opt_epsilon, var_IS_Theory_Analysis, color='b', marker ='*', linewidth=1, markerfacecolor='b', markersize=8, linestyle='--', label='Improved Simmons(thepretical analysis)')
...
p_Simmons = (np.exp(epsilon) - 1)*pi_B / ((np.exp(epsilon) - 1)*pi_B + 1)
# p_Simmons = (np.exp(epsilon) - 1)*(1-pi_B) / ((np.exp(epsilon) - 1)*(1-pi_B) + 1)
var_IS_Numerical_Simulation = np.zeros(len(epsilon))
...
for j in range(len(epsilon)):
    ...
    var_IS_Numerical_Simulation[j] = np.var(mean_proportion)

plt.plot(opt_epsilon, var_IS_Numerical_Simulation, color='b', marker ='o', markerfacecolor='none', linewidth=1, markersize=10, linestyle='--', label='Improved Simmons(numerical simulation)')
plt.xlim(0.2,1)
...
with open('Experimental_verification_of_theory_MS.txt', 'w') as file:
    ...
    for i in range(len(epsilon)):
        line = f"{epsilon[i]}\t{var_IS_Theory_Analysis[i]}\t{var_IS_Numerical_Simulation[i]}\n"  # 使用制表符分隔两列数据
        file.write(line)
