...
'''
improved Simmons, theoretical analysis
'''
pi_A = 0.05
pi_B = 0.5
N = 10000
...
var_IS_Theory_Analysis = np.zeros(num_of_epsilon)
for i in range(num_of_epsilon):
    p_Simmons = (np.exp(epsilon[i])-1)/(np.exp(epsilon[i])+1)
    var_IS_Theory_Analysis[i] = (1-p_Simmons)/p_Simmons/(N-1)*(-pi_B**2+pi_B +pi_A - pi_A**2)
plt.plot(epsilon, var_IS_Theory_Analysis, color='g', marker ='*', linewidth=1, markerfacecolor='g', markersize=8, linestyle='--', label='improved Simmons(theoretical analysis)')
...
plt.plot(epsilon, var_IS_numerical_simulation, color='g', marker ='^', linewidth=1, markerfacecolor='none', markersize=10, linestyle='dashed', label='modified Christofides(L=3, numerical simulation)')
...
with open('Experimental_verification_of_theory_IS.txt', 'w') as file:
