...
'''
Modified Christofides, L=3, theoretical analysis
'''

L = 3
opt_epsilon = [0] * num_of_epsilon
var_MC_Theory_Analysis_L3 = [0] * num_of_epsilon
for i in range(num_of_epsilon):
    ...
    var_MC_Theory_Analysis_L3[i] = DY[A, B] / (L + 1 - 2 * EY[A, B]) / (L + 1 - 2 * EY[A, B]) / M
plt.plot(opt_epsilon, var_MC_Theory_Analysis_L3, color='g', marker ='*', linewidth=1, markerfacecolor='g', markersize=8, linestyle='--', label='modified Christofides(L=3, theoretical analysis)')

'''
Modified Christofides, L=3, numerical simulation
'''
input_list = np.zeros(N)
input_list[:round(pi_A*N)] = 1
REPEAT_TIMES = 10000
var_MC_numerical_simulation_L3 = [0] * num_of_epsilon
...
for i in range(num_of_epsilon):
    ...
    card_draw = np.zeros((REPEAT_TIMES, N))
    rand1 = np.random.rand(REPEAT_TIMES, N)
    ...
    mean_proportion = (np.mean(answer, axis=1) - EY[A, B]) / (L + 1 - 2 * EY[A, B])

    var_MC_numerical_simulation_L3[i] = np.var(mean_proportion)
plt.plot(opt_epsilon, var_MC_numerical_simulation_L3, color='g', marker ='^', linewidth=1, markerfacecolor='none', markersize=10, linestyle='dashed', label='modified Christofides(L=3, numerical simulation)')
...
with open('Experimental_verification_of_theory_MC_L=3.txt', 'w') as file:
    for i in range(len(opt_epsilon)):
        line = f"{opt_epsilon[i]}\t{var_MC_Theory_Analysis_L3[i]}\t{var_MC_numerical_simulation_L3[i]}\n"  
        file.write(line)
